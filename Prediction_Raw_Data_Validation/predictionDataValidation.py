                ...
                if (re.match(regex,filename)):
                    ...
                    if len(splitAtDot[1])==LengthOfDateStampInFile:
                        if len(splitAtDot[2])==LengthOfTimeStampInFile:
                            shutil.copy("Prediction_Data_Files/"+filename,"Prediction_Raw_files_validated/Good_Raw")
                            self.logger.log(f,"Valid file name! File moved to GoodRaw Folder: %s"% filename)
                        else:
                            shutil.copy("Prediction_Data_Files/"+filename,"Prediction_Raw_files_validated/Bad_Raw")
                            self.logger.log(f,"Invalid file name! File moved to Bad Raw Folder: %s"% filename)
                    else:
                        shutil.copy("Prediction_Data_Files/"+filename,
